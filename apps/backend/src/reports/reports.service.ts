...
export class ReportsService {
  ...
  async generatePdf(report: { metadata: ReportMetadata; data: unknown }): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      document.fontSize(20).fillColor('#17324d').text('Mini-SOC Report', { align: 'center' });
      document.moveDown(0.5);
      document.fontSize(10).fillColor('#444');
      document.text(`Type: ${report.metadata.type}`);
      document.text(`Generated at: ${report.metadata.generatedAt}`);
      document.text(`Generated by: ${report.metadata.generatedBy}`);
      document.text(
        `Period: ${new Date(report.metadata.period.start).toISOString()} – ${new Date(report.metadata.period.end).toISOString()}`,
      );
      document.moveDown();
      document.fontSize(13).fillColor('#17324d').text('Report data');
      document.moveDown(0.35);
      document.fontSize(9).fillColor('#222');
